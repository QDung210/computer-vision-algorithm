...
# ================================================================================================================
def preprocess(path='image.jpg', size=(500, 500)):
    img = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
    img = cv2.resize(img, size)
    blur = cv2.GaussianBlur(img, (5, 5), 0)
    grad = cv2.Sobel(blur, cv2.CV_64F, 1, 0, ksize=3)**2 + \
           cv2.Sobel(blur, cv2.CV_64F, 0, 1, ksize=3)**2
    grad = cv2.normalize(np.sqrt(grad), None, 0, 255, cv2.NORM_MINMAX)
    return img, grad
def create_circle_contour(center=(260, 260), radius=400, points=10000, shape=(500, 500)):
...
def find_edges(contour, grad, center=(260, 260), threshold=30):
    dest = []
    for x0, y0 in contour:
        for t in np.linspace(0, 1, 100):
            x, y = int((1 - t) * x0 + t * center[0]), int((1 - t) * y0 + t * center[1])
...
def watershed():
    ...
    plt.subplot(232)
    _, imgThreshold = cv2.threshold(img, 120, 255, cv2.THRESH_BINARY)
...
# ============================================= Thuật toán K-Mean ================================================
# ================================================================================================================
def kmeans_segmentation(image_path = "leaf.jpg", n_clusters=4):
...
if __name__ == "__main__":
    ...
    # =============================================== Thuật toán K-mean ==============================================
    segmented_image = kmeans_segmentation()
